    ...
    if (Test-Command "wsl") {
        ...
        try {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-WarningMessage " ⚠️  WSL may not be fully configured"
            }
        } catch {
            Write-WarningMessage " ⚠️  Unable to verify WSL status"
        }
    } else {
        Write-WarningMessage "⚠️  WSL not found. Install WSL for full AI development experience."
...
function Copy-VSCodeSettings {
    ...
    if (Test-Path $vscodeDir) {
        ...
        if (Test-Path $settingsFile) {
            ...
            Write-WarningMessage "⚠️  settings.json not found"
        }
        
        if (Test-Path $extensionsFile) {
            Write-Success "✓ extensions.json found"
        } else {
            Write-WarningMessage "⚠️  extensions.json not found"
        }
        
        if (Test-Path $tasksFile) {
            Write-Success "✓ tasks.json found"
        } else {
            Write-WarningMessage "⚠️  tasks.json not found"
        }
    } else {
...
function Test-GitConfiguration {
    ...
    if (Test-Command "git") {
        ...
        if ($gitUser -and $gitEmail) {
            ...
            Write-WarningMessage "⚠️  Git user not fully configured"
        ...
        Write-WarningMessage "⚠️  Git not found. Install Git for version control."
    }
}
